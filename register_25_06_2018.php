		...
		<div class="col_one_third nobottommargin">
			...
			<div class="nobottommargin">
				<form id="register-form" name="register-form" class="nobottommargin" action="#" method="post">
					...
					<p class="d-error error_emailRepeat"></p>
					
					</div>
					<div class="col_full">
						<label for="register-form-password">First Name:</label>
						<input type="text" id="register-form-user_first_name" name="user_first_name" value="" class="form-control firsName">
					...
					<div class="d-error error_passwordRepeat"></div>
					</div>
 ...
 //var onwer = document.getElementsByName("onwer");
	...
	if ($('.userEmail').val() == '')
       ...
       if ($('.userEmail').val() != '')
		...
		/*  if ($('.emailRepeat').val() == '')
        {
		     $(".error_emailRepeat").text("Please repeate  email address");
			 valid = false;
        ...
        { 
		   var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
            if (reg.test($('.emailRepeat').val()) == false) 
            {
	...
	if($('.userEmail').val() !=$('.emailRepeat').val()){
		     $(".error_emailRepeat").text("email does not match!");
			 valid = false;
		} 
	  ...
	  if ($('.firsName').val() == '')
        {
         $(".error_firsName").text("Please enter first name");
		  valid = false;
        }	
        ...
        {
		     $(".error_passwordRepeat").text("Please confirm password");
			 valid = false;
           
        }
		
		if($('.passwordNew').val() !=$('.passwordRepeat').val()){
		     $(".error_passwordRepeat").text("Password does not match!");
			 valid = false;
		} 
